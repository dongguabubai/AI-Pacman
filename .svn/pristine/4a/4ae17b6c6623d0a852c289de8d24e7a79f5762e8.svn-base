...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for dis in livingGhostPositionDistributions:
      ghostPos = dis.sortedKeys()[0]
      dis = self.distancer.getDistance(pacmanPosition, ghostPos)
      if dis < curDis:
        curDis = dis
        nextGhostPos = ghostPos
    shorten = -float('inf')
    for action in legal:
      successorPosition = Actions.getSuccessor(pacmanPosition, action)
      nextDis = self.distancer.getDistance(successorPosition, nextGhostPos)
      if(shorten < curDis - nextDis):
        shorten = curDis - nextDis
        nextAct = action
    return nextAct
