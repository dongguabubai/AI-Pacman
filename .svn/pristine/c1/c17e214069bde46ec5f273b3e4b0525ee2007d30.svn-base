...
class ExactInference(InferenceModule):
  ...
  def elapseTime(self, gameState):
    ...
    for p in self.legalPositions:
      ...
      for newPos, prob in newPosDist.items():
        newBeliefs[newPos] += self.beliefs[p]*prob
...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:
      ...
      for particle in self.particles:
        ...
        newBeliefs[particle] += emissionModel[dis]
      newBeliefs.normalize()
      if newBeliefs.totalCount() == 0:
        self.initializeUniformly(gameState)
      else:
        newParticles = [util.sampleFromCounter(newBeliefs) for i in range(self.numParticles)]
  ...
  def elapseTime(self, gameState):
    ...
    newParticles = []
    for particle in self.particles:
      newParticleDist = self.getPositionDistribution(self.setGhostPosition(gameState, particle))
      newParticles.append(util.sampleFromCounter(newParticleDist))
    self.particles = newParticles
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for i in range(self.numParticles):
      each = [random.choice(self.legalPositions) for j in range(self.numGhosts)]
      self.particles.append(tuple(each))
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      for i in range(self.numGhosts):
        ...
        temp.append(newPos)
      newParticles.append(tuple(temp))
    self.particles = newParticles
  ...
  def observeState(self, gameState):
    ...
    for particle in self.particles:
      ...
      for i in range(self.numGhosts):
        ...
        update.append(tuple(newParticle))
    ...
    for particle in self.particles:
      ...
      for i in range(self.numGhosts):
        if noisyDistances[i] != None:
          dis = util.manhattanDistance(newParticle[i], pacmanPosition)
          jointProb *= emissionModels[i][dis]
      newBeliefs[particle] += jointProb
    newBeliefs.normalize()
